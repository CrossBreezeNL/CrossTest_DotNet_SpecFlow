...
namespace CrossBreeze.CrossTest.SpecFlow.Modules.Result
{
    public static class Result_Helper
    {
        ...
        )
        {
            ...
            Assert.IsNotNull(ResultContext.GetResultContext().ResultTable, "There are no results yet to compare with!");

            // Get the column types of the result set.
            Dictionary<String, Type> actualResultTypes = DataHelper.GetColumnTypesFromDataTable(ResultContext.GetResultContext().ResultTable);
            ...
            DataTable differencesTable = DataTableHelper.CompareDataTables(expectedDataTable, ResultContext.GetResultContext().ResultTable);
